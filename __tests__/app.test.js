...
describe('application routes', () => {
  it('has a home GET route that displays a welcome message', () => request(app)
    .get('/')
    .then(res => expect(res.text).toEqual('Welcome to the cars database'))
  );
  
  it('has a POST route to add a car to the database', () => request(app)
    .post('/api/post')
    .send({
      make: 'Ford',
      model: 'Fiesta',
  ...
  it('has a GET route that returns all cars', () => request(app)
    .get('/api/cars')
    .then(res => expect(res.body).toEqual(expect.any(Array)))
  );

  it('has a GET route that gets a car by id', () => request(app)
    .get(`/api/cars/${testCar._id}`)
    .then(res => expect(res.body).toEqual(expect.any(Object)))
  );

  it('has a PUT route that updates a car by id', () => request(app)
